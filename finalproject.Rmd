...
The next check for the MCMC sampling methodolgy is the R-hat estimator of variance, which also helps monitor convergence across the chains (James, 2019). This is shown below using the simple summary command on the model. We see that the R-hat values for each parameter equals 1, implying convergence. There are a few other relevant findings from the model summary that will be discussed later on in this paper. 
...
Now that we understand the posterior model and are confident that the MCMC sampling methodology worked 
